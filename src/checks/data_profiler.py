...
def calculate_dataset_summary(df):
    """
    Calculate dataset-level profiling information.
    """
...
def detect_column_type(
    ...
    is_possible_date_column = any(
        keyword in column_name_lower
        for keyword in date_keywords
    )

    if is_possible_date_column:
...
def calculate_column_profile(df):
    """
    Create profiling information for each dataset column.
    """
    ...
    for column in df.columns:
        ...
        if detected_type in numeric_types:
